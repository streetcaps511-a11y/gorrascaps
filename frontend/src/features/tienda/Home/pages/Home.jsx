...
const Home = () => {
  ...
  useEffect(() => {
    const fetchProductos = async (isFocus = false) => {
      // 🚀 LIMPIEZA DE SEGURIDAD: Si detectamos datos viejos, limpiamos caché
      ...
      try {
        ...
        if (resData.status === 'success' && resData.data.products) {
          const productosDatabase = resData.data.products.map((product) => ({
            ...
            precioOferta: Number(product.precioOferta || 0),
  ...
  const ofertas = useMemo(
    () =>
      initialProducts
        .filter((p) => p.enOferta && p.isActive !== false)
        .slice(0, 8),
    [initialProducts]
  );
  ...
  const destacados = useMemo(
    () =>
      initialProducts
        .filter((p) => p.destacado && p.isActive !== false)
        .slice(0, 8),
    [initialProducts]
  );
  ...
  const ProductCard = ({ product, badge, badgeType }) => {
    ...
    const isOffer = product.enOferta && product.precioOferta;
